...
class FabricNetwork {
  ...
  copyChainCode(source) {
    ...
    shell.cp(source, `${dest}/${path.basename(source)}`);

    // 查询是否有索引文件，如果有，拷贝索引文件到指定目录
    find.file(/\.json$/, path.dirname(source), files => {
      if (files.length) {
        const indexDest = `${dest}/META-INF/statedb/couchdb/indexes`;
        if (!fs.existsSync(indexDest)) shell.mkdir('-p', indexDest);
        files.forEach(file => {
          shell.cp(file, `${indexDest}/${path.basename(file)}`);
        });
      }
    });
  }
