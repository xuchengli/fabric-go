...
import { wait, zipDirectoryFiles, upload, outputGenesisBlock } from './utils';
...
class FabricNetwork {
  constructor() {
    ...
    this.genesisBlockPath = path.join(__dirname, '../artifacts/genesis.block');
  ...
  clean() {
    ...
    shell.rm([`${this.mspDir}.zip`, `${this.genesisBlockPath}`]);
  }
  async startup() {
    // 启动CA
    ...
    const archiveFileName = await zipDirectoryFiles(this.mspDir);
    ...
    // 上传msp文件
    await upload('./msp', archiveFileName);
    ...
    fs.writeFileSync(this.genesisBlockPath, genesis);
