...
import { wait, removeSubDir, buildMSPConfigYaml } from './utils';
...
class FabricNetwork {
  ...
  clean() {
    if (fs.existsSync(this.caDir)) removeSubDir(this.caDir, ['fabric-ca-server-config.yaml']);
  ...
  async startup() {
    ...
    // 注册，发行身份证书，生成pki
    await this.issueIdentity('admin', 'admin', 'org1');
    await this.issueIdentity('peer', 'peer', 'org1');
    await this.issueIdentity('orderer', 'orderer', 'org1');
    await this.issueIdentity('user', 'client', 'org1', [{
      name: 'appAdmin',
      value: true,
    }, {
      name: 'email',
