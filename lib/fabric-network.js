...
class FabricNetwork {
  ...
  async startChainCode(name) {
    const containerId = shell.exec(
      'docker ps -a -f status=running -f name=chaincode --format "{{.ID}}"',
      { silent: true }
    ).stdout.trim();
    if (!containerId) {
      alert('chaincode container is not available.');
      return;
    }
    ...
    return new Promise((resolve, reject) => {
      container.exec({ Cmd: ['/bin/bash'], AttachStdin: true, AttachStdout: true, AttachStderr: true }, (err, exec) => {
        ...
        exec.start({ hijack: true, stdin: true }, (err, stream) => {
          ...
          stdin.push(`cd ${name} && go build`);
          stdin.push(null);
          stdin.pipe(stream);

          const stdout = new Writable();
          stdout.on('finish', chunk => {
