...
class FabricNetwork {
  ...
  async startup() {
    ...
    // 在admin, user, peer, orderer的msp目录创建node OU configuration（msp/config.yaml）
    buildMSPConfigYaml(path.join(this.caClientsDir, 'admin/msp'));
    buildMSPConfigYaml(path.join(this.caClientsDir, 'user/msp'));
    buildMSPConfigYaml(path.join(this.caClientsDir, 'peer/msp'));
    buildMSPConfigYaml(path.join(this.caClientsDir, 'orderer/msp'));
