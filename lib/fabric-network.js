...
class FabricNetwork {
  ...
  async startup() {
    ...
    // 注册，发行身份证书，生成pki
    await this.issueIdentity('admin', 'admin', 'zhigui');
    await this.issueIdentity('peer', 'peer', 'zhigui.beijing');
    await this.issueIdentity('orderer', 'orderer', 'ibm.crl.it');
    await this.issueIdentity('user', 'client', 'ibm.cdl.dev', [{
      name: 'appAdmin',
      value: true,
    }, {
      name: 'email',
  ...
  async issueIdentity(name, role, affiliation, attrs = []) {
    ...
    // 发行证书
    const enroll_cmd = `fabric-ca-client enroll -u http://${name}:${name}@localhost:7054 -M /root/ca-clients/${name}/msp`;
