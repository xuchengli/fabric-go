...
export default class FabricGoView {
  ...
  constructor(serializedState) {
    ...
    const chaincodeList = document.createElement('ol');
    chaincodeList.classList.add('list-group');
    chaincodeInsetPanelBody.appendChild(chaincodeList);

    const chaincodes = this.fabricNetwork.listChainCode();
    chaincodes.forEach(chaincode => {
      const chaincodeItem = document.createElement('li');
      chaincodeItem.classList.add('two-lines');
      chaincodeList.appendChild(chaincodeItem);

      const chaincodeItemText = document.createElement('div');
      chaincodeItemText.classList.add('primary-line', 'icon', 'icon-file-text');
