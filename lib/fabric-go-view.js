...
export default class FabricGoView {
  ...
  changeStatus() {
    if (!shell.which('docker-compose')) {
      alert('Sorry, please install docker compose first.');
      return;
    }
    const composeFile = path.join(__dirname, '../artifacts/docker-compose.yaml');
    ...
    if (this.status === 'up') {
      ...
      // TODO: 启动网络
      shell.exec(`docker-compose -f ${composeFile} up -d`, { silent: true }, (code, stdout, stderr) => {
