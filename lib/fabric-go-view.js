...
import InstallCCModal from './install-cc-modal';
    ...
    this.identityInsetPanelHeaderButton.addEventListener('click', () => {
      ...
      const modalPanel = atom.workspace.addModalPanel({
        item: new IdentityModal(
          this.fabricNetwork.caServerConfig.affiliations,
          (instance, identity) => {
    ...
    this.chaincodes.forEach(chaincode => {
      if (!this.chaincodeMap[chaincode]) {
        ...
        chaincodeItemBuildButton.addEventListener('click', () => {
          ...
          const cmd = `pgrep ${chaincode} | xargs -r kill -9 && cd ${chaincode} && go build && CORE_PEER_ADDRESS=peer0.zhigui.com:7052 CORE_CHAINCODE_ID_NAME=${chaincode}:0 ./${chaincode}`;
        ...
        chaincodeItemInstallButton.addEventListener('click', () => {
          ...
          //安装到不同节点的输入窗口
          const nodes = Object.entries(this.fabricNetwork.networkConfig.services).filter(([k, v]) => {
            return v.image.includes('hyperledger/fabric-peer');
          }).map(([k, v]) => k);
          const modalPanel = atom.workspace.addModalPanel({
            item: new InstallCCModal(
              nodes,
              node => {
                const nodeIdentity = this.fabricNetwork.networkConfig.services[node]['environment'].filter(env => {
                  return env.startsWith('CORE_PEER_ADDRESS') || env.startsWith('CORE_PEER_LOCALMSPID');
                }).reduce((acc, cur) => {
                  let [k, v] = cur.split('=');
                  if (k === 'CORE_PEER_ADDRESS') v = `${v.split(':')[0]}:7051`;
                  return Object.assign(acc, { [k]: v });
                }, {});
                const org = node.split('.')[1];
                Object.assign(nodeIdentity, {
                  'CORE_PEER_MSPCONFIGPATH': `/etc/hyperledger/ca-clients/${org}.com/users/admin@${org}/msp`,
                });
                modalPanel.destroy();
                ...
                // 先删除已经安装的链码
                this.fabricNetwork.exec(node, `rm -rf /var/hyperledger/production/chaincodes/${chaincode}.0`, (err, msg) => {
                  if (err) {
                    chaincodeItemInstallButton.innerHTML = 'Install';
                  ...
                  // 再安装当前的链码
                  const cmd = `CORE_PEER_ADDRESS=${nodeIdentity['CORE_PEER_ADDRESS']} ` +
                              `CORE_PEER_LOCALMSPID=${nodeIdentity['CORE_PEER_LOCALMSPID']} ` +
                              `CORE_PEER_MSPCONFIGPATH=${nodeIdentity['CORE_PEER_MSPCONFIGPATH']} ` +
        ...
        logConsoleBody1.classList.add('console-body', 'native-key-bindings');
        ...
        logConsoleBody2.classList.add('console-body', 'native-key-bindings');
